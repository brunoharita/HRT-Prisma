-- Exact canonical reuse is required by the global unique index. It is not a
-- semantic/fuzzy merge: scope, concept type and normalized canonical label
-- must all agree before an additional official source can map to a concept.
...
begin
  ...
  where stage.source_version_id = p_source_version_id
    ...
    and not exists (
      select 1 from public.knowledge_external_mappings mapping
      where mapping.source_version_id = stage.source_version_id
