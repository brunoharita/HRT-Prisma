-- Forward-only normalization of historical fallback profiles during document deletion.
  ...
  if current_profile.id is not null and current_profile.source_document_id = original_document_id then
    ...
    if fallback_profile.id is not null then
      ...
      insert into public.professional_profiles (
        ...
        private.normalize_profile_review_contract(fallback_profile.profile_data, fallback_profile.profile_data),
...
comment on function public.finalize_document_deletion(uuid, uuid) is
  'Finalizes Storage deletion and rebuilds the current profile through the current conservative contract normalizer.';
