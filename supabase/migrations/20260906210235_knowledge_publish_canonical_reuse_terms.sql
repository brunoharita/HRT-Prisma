-- Forward-only completion for exact canonical reuse. Candidate stage records
-- are omitted from the legacy publisher loop only after their immutable
-- source mappings have been created; their canonical and alias terms retain
-- ESCO provenance and all staged relations remain eligible for publication.
...
begin
  ...
  select stage.*
  from public.knowledge_source_stage_records stage
  join public.knowledge_concepts concept
    on concept.scope = 'global'
   and concept.organization_id is null
  ...
  select concept.id, stage.source_id, stage.source_version_id, stage.external_id, stage.external_uri,
    'exact', jsonb_build_object('source_file', stage.source_file, 'source_row', stage.source_row,
  ...
  update public.knowledge_source_stage_records stage
  set source_status = 'canonical_reused'
  where stage.source_version_id = p_source_version_id
    and stage.record_kind = 'concept'
  ...
  select mapping.concept_id, 'global', null, stage.preferred_label,
    private.normalize_knowledge_term(stage.preferred_label), stage.language, 'canonical',
    stage.source_id, stage.source_version_id, 'approved', 1
  from m55_canonical_reuse stage
  ...
  select mapping.concept_id, 'global', null, alias.value,
    private.normalize_knowledge_term(alias.value), stage.language, 'alias',
    stage.source_id, stage.source_version_id, 'approved', 1
  from m55_canonical_reuse stage
  join public.knowledge_external_mappings mapping
    on mapping.source_version_id = stage.source_version_id and mapping.external_id = stage.external_id
