...
begin
  ...
  if exists (
    ...
    where (descriptor.value ->> 'fieldPath') is null
       ...
       or ((descriptor.value ->> 'x') is not null and not (
         (page.value ->> 'origin' = 'native_pdf' and descriptor.value ->> 'method' = 'pdfjs-layout-v1')
         or (page.value ->> 'origin' = 'ocr' and descriptor.value ->> 'method' = 'tesseract-layout-v1')
       ))
...
comment on function public.persist_person_extraction(uuid, uuid, uuid, jsonb, jsonb, integer, integer, text, text, text, text, text, uuid) is
  'Persists one tenant-scoped extraction atomically and accepts bounded spatial field evidence only when its method matches the native PDF or OCR page origin.';
