-- M5.4.5 hotfix: a chosen occupation reference complements, never replaces,
-- the original assisted-description provenance.
...
begin
  ...
  update public.vacancy_versions set structure_source = jsonb_build_object('originalDescription',v_original,'contractVersion',v_contract,'structuredAt',coalesce(p_source->>'structuredAt',now()::text),'items',v_items), source_kind = 'assisted_description', contract_version = 'vacancy-definition-2.0.0'
  where id=p_vacancy_version_id and organization_id=p_organization_id and source_kind in ('assisted_description', 'knowledge_reference');
