-- Forward repair for the single remote environment, where the historical M5.6
-- observability migration was not applied. Idempotent after a full migration replay.
...
create table if not exists public.document_intelligence_runs (
  ...
  stage_metrics jsonb not null check (jsonb_typeof(stage_metrics) = 'array' and jsonb_array_length(stage_metrics) between 1 and 32),
...
create index if not exists document_intelligence_runs_document_idx
...
begin
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'document_intelligence_runs' and policyname = 'document_intelligence_runs_select'
  ) then
    execute $policy$
      create policy document_intelligence_runs_select
      on public.document_intelligence_runs for select to authenticated
  ...
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'document_intelligence_runs' and policyname = 'document_intelligence_runs_insert'
  ) then
    execute $policy$
      create policy document_intelligence_runs_insert
      on public.document_intelligence_runs for insert to authenticated
      with check (
...
alter table public.organization_extraction_patterns
  add column if not exists structural_signature_version text not null default 'experience-block-signature-v3'
    check (char_length(structural_signature_version) between 8 and 120),
  add column if not exists provider_family text not null default 'provider-neutral'
    check (provider_family in ('provider-neutral', 'pdfjs', 'tesseract', 'paddleocr')),
  add column if not exists applicability jsonb not null default '{"documentKinds":["resume"],"coordinateSystem":"normalized-page-v1"}'::jsonb
...
where status = 'retired' and (retired_at is null or invalidation_reason is null);

do $$
begin
  if not exists (
    select 1 from pg_constraint where conrelid = 'public.organization_extraction_patterns'::regclass and conname = 'organization_extraction_patterns_retirement_metadata_check'
  ) then
    alter table public.organization_extraction_patterns
...
comment on table public.document_intelligence_runs is
  'PII-free routing, provider version and stage metrics for document intelligence.';
