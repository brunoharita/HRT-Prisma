...
function scoreArea(applicable, relation) {
    if (!applicable)
        return emptyDimension("area", "A Posição não definiu área profissional; os 30 pontos ficam fora do denominador.");
...
function scoreFunction(applicable, assessment) {
    if (!applicable)
        return emptyDimension("position", "A Posição não definiu função; os 20 pontos ficam fora do denominador.");
...
function validateVersions(input, scoreContractVersion) {
    ...
    if (scoreContractVersion !== (semantic ? "matching-score-1.3.0" : MATCHING_SCORE_CONTRACT_VERSION))
