...
export function prepareTrajectoryContext(profileData, position, redactions = []) {
    const profile = record(profileData);
    const experiences = array(profile.experiences).map(record);
    if (experiences.length > 60)
    ...
    const clean = (value) => {
        ...
        for (const name of names.sort((a, b) => b.length - a.length))
            text = text.replace(new RegExp(`(?<![\\p{L}\\p{N}_])${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?![\\p{L}\\p{N}_])`, "giu"), "[omitido]");
        return text.replace(/https?:\/\/\S+|www\.\S+|[\w.+-]+@[\w.-]+\.[a-z]{2,}/gi, "[contato omitido]")
            ...
            .split(/[;\r\n]+/).filter(clause => {
            if (!/\b(idade|nasciment|casad[oa]|solteir[oa]|relig[iã]|defici[eê]ncia|gestante|gr[aá]vida|sexual|etnia|racial)\w*/i.test(clause))
                return true;
            ...
            if (/^\s*idade\s*:\s*\d{1,3}\s*$/i.test(clause)
                ...
                || /^\s*(?:gestante|gr[aá]vida|casad[oa]|solteir[oa])\s*$/i.test(clause))
                return false;
    ...
    experiences.forEach((item, index) => {
        ...
        if (text)
            entries.push({ id: `e${index}`, fieldPath: `experiences.${index}`, text, kind: "experience" });
    });
    const title = clean(profile.professionalTitle);
    if (title)
        entries.push({ id: "title", fieldPath: "professionalTitle", text: title, kind: "declaration" });
    const area = array(profile.areasOfExpertise).map(clean).filter(Boolean).join("; ");
    if (area)
        entries.push({ id: "area", fieldPath: "areasOfExpertise", text: area, kind: "declaration" });
    ...
    if (context.position.length > 6000 || entries.some(item => item.text.length > 4000) || JSON.stringify(context).length > 24000)
        throw new Error("TRAJECTORY_INPUT_LIMIT");
    return context;
}
...
export function readTrajectoryResponse(value, context) {
    ...
    const items = data.items.map(item => {
        ...
        if (Object.keys(row).sort().join() !== "activity,id,quote" || !entry || seen.has(id) || typeof row.quote !== "string"
            || !activities.includes(row.activity) || quote.length > 4000
            || (row.activity !== "unclear" && quote.trim().length < Math.min(6, entry.text.length)) || !entry.text.includes(quote))
            throw new Error("TRAJECTORY_RESPONSE_INVALID");
        seen.add(id);
        return { id, activity: row.activity, quote };
    });
