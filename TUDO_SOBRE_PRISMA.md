...
source_manifest_sha256: 73c3c1e062c9c8cf4c523f014275ffa3308faf384e7d9ef6c0223abc527317bd
-->
...
- M5.4 Vagas 1.0.0 implementada localmente e ativa no Prisma-QA: Vaga representa uma necessidade profissional versionada de posição ocupada ou não ocupada, distinta de função, referência ocupacional e campanha de recrutamento. A jornada inclui lista, criação manual ou por referência, estruturação determinística de descrição, detalhe editorial, descoberta por Perfil publicado e comparação de exatamente duas Pessoas. Matching separa evidência direta, equivalência Knowledge, inferência rastreável, sinal relacionado específico da versão e insuficiência de evidência, sem score, ranking ou vencedor. `save_vacancy_definition` preserva snapshots e requisito estável, valida tenant/papel/Pessoa/Knowledge e mantém DML direto revogado; Member e anon não recebem autoridade. A lista expõe edição e exclusão; `cancel_vacancy` retira a Vaga da lista por cancelamento auditável, sem apagar posição, Pessoa, versões ou avaliações. A prova remota revertida confirmou v1/v2, cancelamento auditável, grants negativos e rejeição de posição planejada/inativa, e o advisor encerrou sem nova FK sem cobertura. O smoke autenticado `1280x720` aprovou as seis superfícies; quatro capturas adicionais permanecem pendentes porque o harness disponível nesta rodada manteve viewport fixa. O registro sintético, sua posição e sua função exclusivas foram removidos após a prova. O gate completo aprovou lint de 322 arquivos, Context Pack, dois typechecks, build web, 249 testes técnicos, 19 casos golden e `VERTICAL_SLICE_OK`.
...
- M5.4.3 Resolução ocupacional sob demanda está ativa no Prisma-QA: ao concluir o título de uma Vaga, o Prisma reutiliza primeiro aliases e reconciliações Knowledge publicadas e, quando não há decisão segura, consulta no máximo doze candidatos dos snapshots oficiais em staging. `occupation_resolution_attempts` preserva termo normalizado, versões, candidatos, motivos e chave idempotente sob RLS tenant-scoped; nenhuma fonte é publicada por essa consulta, nenhum dado de Pessoa participa e falha/ambiguidade não bloqueiam o preenchimento manual. O contrato é `occupation-resolution-on-demand-1.0.0`; o `knowledge-agent` continua a única fronteira externa para exceções futuras e não foi ativado nesta entrega.
