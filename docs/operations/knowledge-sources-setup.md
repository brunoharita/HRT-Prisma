...
Na Home, cada base central possui o botão `Checar agora`, visível ao Super Admin. A ação reutiliza a Edge Function `knowledge-source-monitor` com `trigger=manual` e o ID da base; a função valida o JWT, o operador ativo e o perfil `super_admin` antes de consultar a fonte oficial. A checagem apenas atualiza o estado e o ledger de monitoramento; não publica uma nova versão.
