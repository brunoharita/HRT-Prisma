...
Evidência atual em 2026-09-13: o schema funcional acumulado e a migration mais recente `20260913132559` estão ativos no Prisma-QA. As dez Edge Functions presentes no repositório foram reimplantadas e confirmadas `ACTIVE`. O histórico remoto de migrations anterior a setembro contém timestamps diferentes dos arquivos locais equivalentes; a simulação oficial falha fechada sem executar SQL. Não reparar o ledger nem reaplicar migrations antigas sem um movimento específico de reconciliação com prova de equivalência. O frontend adaptativo permanece local porque não existe hosting remoto; o smoke visual autenticado continua obrigatório antes de um rollout futuro.
