...
- O contrato `resume-product-state` 1.0.0 deriva sete estados visíveis a partir dos fatos técnicos: `Processando`, `Requer identificação`, `Requer revisão`, `Pronto para publicação`, `Perfil atualizado`, `Falha técnica` e `Descartado`.
