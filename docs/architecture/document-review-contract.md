...
| `person-ingestion` | 5.1.0 | cadastro, retry e persistência são idempotentes; intake resolve a Pessoa; extração e revisão consomem layout, evidência por campo, áreas personalizadas e padrões aprovados do tenant |
...
| `human-profile-review` | 2.1.0 | revisão possui rascunho, releitura imediata por bloco, áreas personalizadas com evidência por item, aceite parcial atômico, mudanças por campo, validação contextual e exclusão auditável de evidência |
