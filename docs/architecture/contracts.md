...
| `operation-feedback` | application/UI | 2.0.0 | fronteiras Supabase, ingestão, revisão, evidência, Delta, verificações, Item Bank e Conhecimento | envelope ativo em QA; tradutor e apresentação web locais | motivo, campo, item, mensagem natural, recuperação segura e proibição testada de mensagem técnica bruta | local/QA | distinguir pendência humana de falha interna |
| `decision-centered-interaction` | product/application/UI | 1.0.0 | todos os fluxos operacionais | implementado no descarte adaptativo; normativo para novas mudanças | clique ou tecla obrigatórios somente para julgamento, autoridade ou risco material; coordenação e avisos opcionais não bloqueiam | local | preservar dados e não criar bloqueio auxiliar |
...
| `vacancy-definition` | product/application/data | 1.0.0 | Vagas, posições, funções, histórico e matching | ativo no Prisma-QA | snapshots imutáveis, requisito estável e escrita tenant-scoped | local/QA | bloquear escrita ou avaliação sem versão atual |
| `vacancy-matching-explainable` | product/application/domain | 1.1.0 | descoberta e comparação M5.4 | implementação local sobre schema ativo no Prisma-QA | cada requisito consulta todo o Perfil publicado e explica as áreas da evidência; equivalência Knowledge e sinal relacionado permanecem separados, sem score | local/QA | tratar como sem evidência e não ampliar resultado |
| `vacancy-structure-assistant` | product/application/UI | 1.2.0 | criação e revisão contextual M5.4 | ativo em QA; chamada viva bloqueada sem credencial do provider | entrada livre, contexto interno, pesquisa Web automática quando há dependência de atualidade, fontes visíveis e nenhuma alteração sem ação humana | local/QA | preservar análise interna e declarar a falha externa |
| `vacancy-market-research` | AI/security/operations | 1.0.0 | modo `vacancy_advisor` do Knowledge Agent | schema e Edge Function ativos em QA; provider bloqueado sem `OPENAI_API_KEY` | request mínimo no-PII, Web Search allowlisted, Structured Output, fontes pós-validadas, cache 24h, ledger e caps | local/QA | não chamar provider e preservar edição manual |
...
| `adaptive-resume-extraction` | AI/application | 6.1.0 | ingestão M2-B e revisão | sibling discovery v3 ativo em QA; resumo e classificação acadêmica locais | assinatura document-local, blocos irmãos, formação qualificada, resumo seccionado, geometria e IDs estáveis | local/QA | exigir revisão/reprocessamento |
...
| `extraction-draft` | AI/domain | 7.1.0 | evidência e geração de perfil | schema v6 ativo em QA; runtime v7.1 local | `prisma-layout-adaptive-v7` + `education-classifier-1.0.0` | local/QA | bloquear promoção |
...
| `knowledge-research` | AI/security | 1.0.0 | Knowledge Agent | adapter ativo em QA; provider bloqueado sem credencial | `knowledge-research-1.0.0` | local/QA | não chamar provider |
