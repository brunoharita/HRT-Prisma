...
| `adaptive-resume-extraction` | AI/application | 2.0.0 | ingestão M2-B e revisão | runtime local; persistência ativa em QA | geometria, releitura por bloco e sinais organizacionais aprovados | local/QA | exigir revisão/reprocessamento |
...
| `extraction-draft` | AI/domain | 3.0.0 | evidência e geração de perfil | implementado localmente | `prisma-layout-adaptive-v2` | local/QA | bloquear promoção |
