...
| `vacancy-definition` | product/application/data | 1.1.0 | Vagas, posições, funções, histórico e matching | ativo no Prisma-QA | snapshots imutáveis, requisito estável, classificação humana e escrita tenant-scoped | local/QA | permitir descoberta e marcar aderência detalhada como pendente quando houver requisito sem classificação |
| `vacancy-matching-explainable` | product/application/domain | 2.1.0 | descoberta e comparação M5.4 | implementação local sobre schema ativo no Prisma-QA | todos os Perfis publicados são paginados e analisados; somente relação ou evidência rastreável gera resultado; posição e requisitos permanecem separados, sem score | local/QA | retornar lista vazia e orientar revisão da Vaga |
...
| `document-intelligence-provider` | AI/infra | 1.0.0 | ingestão M5.6 | implementado localmente, runtime não executado | porta substituível e adaptador Paddle self-hosted | local | retornar ao baseline |
