...
| `vacancy-definition` | product/application/data | 1.1.0 | Vagas, posições, funções, histórico e matching | ativo no Prisma-QA | snapshots imutáveis, requisito estável, classificação humana e escrita tenant-scoped | local/QA | bloquear matching com requisito sem classificação |
