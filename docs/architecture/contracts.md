...
| `vacancy-matching-explainable` | product/application/domain | 1.0.0 | descoberta e comparação M5.4 | ativo no Prisma-QA | evidência direta, equivalência Knowledge e sinal relacionado separados, sem score | local/QA | tratar como sem evidência e não ampliar resultado |
| `vacancy-structure-assistant` | product/application/UI | 1.0.0 | criação assistida M5.4 | ativo no Prisma-QA | preparação determinística, derivação desmarcada e confirmação humana | local/QA | manter edição manual disponível |
