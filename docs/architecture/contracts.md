...
| `person-ingestion` | application/data | 5.2.0 | intake, Pessoas, documentos, perfil | schema ativo em QA; web local | intake, layout visual, evidência refinável por campo, áreas personalizadas e padrões aprovados por tenant | local/QA | bloquear processamento |
...
| `human-profile-review` | application/domain | 2.2.0 | revisão, perfil, auditoria | schema ativo em QA; web local | releitura por bloco, subtração reversível de áreas mapeadas, áreas personalizadas, histórico e exclusão auditável | local/QA | bloquear promoção |
