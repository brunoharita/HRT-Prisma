...
| `vacancy-matching-explainable` | product/application/domain | 2.0.0 | descoberta e comparação M5.4 | implementação local sobre schema ativo no Prisma-QA | todos os Perfis publicados são paginados; posição e requisitos são avaliados separadamente; requisito usa somente sua dimensão, com evidência e decisão humana, sem score | local/QA | preservar Perfil na análise manual e não declarar aderência automática |
