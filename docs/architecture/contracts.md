...
| `vacancy-structure-assistant` | product/application/UI | 1.1.0 | criação e revisão contextual M5.4 | implementação local sobre Knowledge ativa no Prisma-QA | entrada livre, contexto interno, resposta separada entre empresa, mercado e sugestão; Web depende do Knowledge Agent governado e não altera dados sem ação humana | local/QA | manter edição manual e declarar mercado não consultado |
