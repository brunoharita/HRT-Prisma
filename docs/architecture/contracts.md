...
| `profile-document-lifecycle` | product/application/data/security | 1.0.0 | Central da Pessoa, versões, documentos, Storage | ativo no Prisma-QA | restauração aditiva, reset não destrutivo e exclusão retomável por Edge Function | local/QA | preservar estado e oferecer repetição segura |
...
| `person-ingestion` | application/data | 11.0.0 | intake, Pessoas, documentos, perfil | ciclo de vida ativo no Prisma-QA | ingestão, revisão, publicação, restauração, reset e exclusão coordenada | local/QA | bloquear sem fonte recuperável |
...
| `document-operation-idempotency` | application/data | 2.0.0 | cadastro, retry, publicação, restauração, reset e exclusão | ativo no Prisma-QA | `document_operations`, fingerprints, locks e saga retomável de Storage | local/QA | rejeitar conflito |
