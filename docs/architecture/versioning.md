...
A segunda geração adaptativa mantém `pdf-native-extraction` 2.0.0 e `spatial-evidence` 1.1.0, e avança `adaptive-resume-extraction` para 2.0.0, `extraction-draft` para 3.0.0, `person-ingestion` para 5.0.0 e `human-profile-review` para 2.0.0. A releitura imediata por bloco é document-local e exige aceite humano. Somente a aprovação integral da revisão promove sinais estruturais tenant-scoped para a primeira extração futura. Versão de padrão desconhecida é ignorada e nunca muda silenciosamente o significado de um contrato persistido.
