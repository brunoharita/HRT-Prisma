...
| D-22 | Projeto remoto identificado `ioldpnqqvobprjiontre`; backup restaurado; CI, main, VPS e HTTP hospedado conferidos | PARTIAL | Seis exclusões e smoke funcional autenticado pendentes |
...
| D-27 | M8.1 é a primeira entrega registrada do Movimento 8; CI, `main`, VPS e login hospedado confirmam `v1.8.1`; menu lê a mesma fonte | PASS | Menu hospedado não inspecionado em sessão autenticada |
...
- `pnpm run typecheck:web`, `pnpm run build:web`, `pnpm run build`, 11 testes dirigidos de M8/M7.2/M7.6 e 22 regressões dirigidas de M7.1/M7.3/M7.7: PASS. Dois testes antigos de navegação falharam no primeiro CI por esperar ação primária e rota anteriores às telas M8.1; as expectativas foram alinhadas ao produto aprovado, 15 testes dirigidos passaram, e o CI completo do SHA `11ff0ab` passou. O build Vite avisou sobre chunks grandes e import dinâmico ineficaz, sem falha.
...
- Execução remota parcial pela saga M5.5: `[QA] Marina Dados`, Beatriz Galazzini e TAINÁ MARQUES foram excluídas definitivamente; a UI confirmou banco e arquivos. Consulta SQL posterior: 7 Pessoas no total, 6 dos 9 alvos ainda presentes, os 3 IDs concluídos ausentes, 11 objetos Storage, 7 Auth, 7 `platform_users` e 8 Vagas. O Product Owner interrompeu a exclusão individual e pediu continuidade do deploy. Nenhuma outra Pessoa será excluída nesta execução; a limpeza M8.1 permanece `PARTIAL`.
- Publicação: CI `Prisma foundation CI` #35528204365 aprovado para `11ff0ab1d81c3451adb7ae7e928f8923be223932`; `main` e `origin/main` nesse SHA; `/opt/prisma` na VPS no mesmo SHA, contêiner `prisma-web` ativo; `release:verify` confirmou HTTPS 200. As cinco migrations haviam sido aplicadas separadamente no projeto Supabase antes da web. Não houve nova Edge Function. Smoke funcional autenticado da web hospedada e ciclo intake → revisão → publicação permanecem `NOT TESTED`.
- Correção de release: após o Product Owner apontar `v1.7.6` no login e no menu, o registro central recebeu a primeira entrega do Movimento 8. O SHA `4147a369c7b44a3e702d53c630c14eff1f561017` passou no CI #35529506883, foi promovido à `main` e reconstruído na VPS. O primeiro HEAD imediatamente após a troca do contêiner retornou 404 e fez o script encerrar com código 22; segundos depois, HEAD público retornou 200, o contêiner permaneceu ativo e o login hospedado exibiu `v1.8.1`. O menu usa `PRISMA_RELEASE.displayVersion` da mesma fonte, confirmado por teste de código; a renderização hospedada autenticada não foi conferida.
...
Nenhum desvio implementado foi aprovado como substituto de requisito. A limpeza foi interrompida pelo Product Owner após três exclusões; seis alvos e a limpeza dependente de Inbox/aliases permanecem pendentes. A confirmação individual do Product Owner resolveu a origem de `[QA] Marina Dados` apenas para esse cadastro; a outra Pessoa fica preservada. Cinco Inbox vinculadas exclusivamente às observações do lote e dois aliases organizacionais requerem limpeza exata após a saga, com guarda de referências independentes; o conceito organizacional aprovado com proposta própria e os conceitos globais permanecem preservados. `supabase db push` geral e `migration repair` automático continuam vedados pelo ledger histórico.

## Git / QA / produção

Branch `codex/m81-competency-architecture` integrada em `main` no SHA `11ff0ab1d81c3451adb7ae7e928f8923be223932`. Cinco migrations M8.1 estão no banco remoto e a web M8.1 foi publicada na VPS; HTTPS 200 e contêiner ativo verificados. Três dos nove cadastros do lote foram excluídos, seis permanecem por orientação posterior do Product Owner. O smoke funcional autenticado hospedado e o ciclo intake → revisão → publicação não foram executados; limpeza de Inbox/aliases e evidência pós-limpeza total continuam pendentes. O trabalho não pode ser declarado concluído enquanto houver `D-*` bloqueado, parcial ou sem teste.
