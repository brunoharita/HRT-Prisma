...
1. `CI=true pnpm run validate` aprovou 148 testes técnicos, 19 golden, build e demo.
...
6. Smoke visual autenticado ainda pendente. Produção não foi alterada.

## Limitações aceitas

Texto achatado sem coordenadas não cria nova experiência. Layouts heterogêneos, períodos ambíguos, colunas divergentes e candidatos parciais continuam sob revisão manual. Nenhuma alegação de precisão em currículos reais existe antes da amostra autorizada.
