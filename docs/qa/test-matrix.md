...
| Evidência espacial M5 | selecionar somente caracteres visualmente contidos, aplicar texto reconhecido sem justificativa, corrigir/complementar/substituir e navegar no histórico | linha que apenas intersecta a borda, divergência sem justificativa, erro oculto atrás do modal, coordenada/página/versão inválida, Member, cross-tenant e DML direto | regressão local e schema/RPC QA aprovados; browser autenticado pendente |
