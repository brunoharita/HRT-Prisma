...
export function CanonicalProfileView({ profile, compact = false, showHeader = true, showCompetencies = true, onShowEvidence }: CanonicalProfileViewProps) {
  ...
  return (
    <article className={`prisma-canonical-profile${compact ? " is-compact" : ""}`}>
      {showHeader ? <CanonicalProfileHeader profile={profile} /> : null}
      <div className="prisma-canonical-profile__content">
        ...
        {showCompetencies && profile.competencyGroups.length ? <CanonicalSection action={evidenceAction(onShowEvidence, "competencies")} icon={<BulbOutlined />} title="Competências">
