...
import { PARSER_IA_VERSION, structureParserIa } from "../domain/parserIa";
...
export async function prepareParserIa(input: ProcessedDocumentInput, organizationId: string): Promise<ProcessedDocumentInput> {
  ...
  try {
    ...
    return { ...input, pages: output.pages, nativePageCount: output.pages.length, ocrPageCount: 0, parserIa: parsed };
  } catch {
